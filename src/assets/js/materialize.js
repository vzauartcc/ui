...
var M = (function (exports) {
	...
	class Autocomplete extends Component {
		...
		constructor(el, options) {
			...
			this.oldVal = '';
			this.selectedValues =
				this.selectedValues || this.options.selected.map((value) => ({ id: value })) || [];
		...
		static Init() {
			if (typeof document !== 'undefined')
				...
				document.addEventListener('DOMContentLoaded', () => {
					...
					cards.forEach((el) => {
						if (el && el['M_Card'] == undefined) this.init(el);
					});
				});
		}
	}
	...
	class FormSelect extends Component {
		...
		_setupDropdown() {
			...
			this.selectOptions = Array.from(this.el.children).filter((el) =>
			...
			if (this.selectOptions.length > 0) {
				this.selectOptions.forEach((realOption) => {
					if (realOption.tagName === 'OPTION') {
						...
						const groupChildren = [];
						const selectOptions = Array.from(realOption.children).filter(
	...
	class Datepicker extends Component {
		...
		_renderDateInputFormat(el) {
			el.parentElement.querySelector('.datepicker-format').innerHTML =
		...
		render(year, month, randId) {
			...
			for (let i = 0, r = 0; i < cells; i++) {
				const day = new Date(year, month, 1 + (i - before)),
					...
					isEndRange =
						this.options.endRange && Datepicker._compareDates(this.options.endRange, day),
					isInRange =
		...
		static textareaAutoResize(e) {
			...
			if (!textarea.hasAttribute('original-height'))
				textarea.setAttribute(
					'original-height',
					textarea.getBoundingClientRect().height.toString(),
				);
	...
	class TapTarget extends Component {
		...
		_handleDocumentClick = (e) => {
			if (
				e.target.closest(`#${this.el.dataset.target}`) !== this.originEl &&
