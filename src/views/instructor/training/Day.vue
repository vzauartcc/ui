				...
				<tbody class="requests_list_row">
					<tr v-for="(request, i) in requests" :key="request._id">
						<td>
							...
							<span v-if="request.student.vis === true">(VC)</span>
						</td>
						...
						<td class="options">
							<a
								:href="`#modal_request_${i}`"
								data-position="top"
								data-tooltip="View Request"
								class="tooltipped modal-trigger"
							>
								<i class="material-icons" @click="setTimes(i)">search</i>
							</a>
							<a
								:href="`#modal_delete_${i}`"
								data-position="top"
								data-tooltip="Delete Request"
								class="tooltipped modal-trigger red-text text-darken-2"
								v-if="requiresAuth(['atm', 'datm', 'ta', 'ins'])"
							>
							...
							<div class="modal-content">
								...
								<div class="request">
									<div class="row row_no_margin" id="request">
										...
										<div class="input-field col s12 m6">
											<p id="milestone">
												{{ request.milestone.name }} ({{ request.milestone.code }})
											</p>
										...
										<div class="input-field col s12 m6">
											<div id="start_time">
												...
												<div class="controls">
													<div>
														<i class="material-icons" @click="increaseTime('start', i)"
															>arrow_drop_up</i
														>
													</div>
													<div>
														<i class="material-icons" @click="decreaseTime('start', i)"
															>arrow_drop_down</i
														>
													</div>
												</div>
										...
										<div class="input-field col s12 m6">
											<div id="end_time">
												...
												<div class="controls">
													<div>
														<i class="material-icons" @click="increaseTime('end', i)"
															>arrow_drop_up</i
														>
													</div>
													<div>
														<i class="material-icons" @click="decreaseTime('end', i)"
															>arrow_drop_down</i
														>
													</div>
												</div>
						...
						<div :id="`modal_delete_${i}`" class="modal modal_request">
							<div class="modal-content">
								...
								<b>{{ request.student.fname + ' ' + request.student.lname }}</b
								>'s training request from <b>{{ dtLong(request.startTime) }}</b> until
								<b>{{ dtLong(request.endTime) }}</b
								>?
							</div>
							<div class="modal-footer">
								<a
...
export default {
	...
	methods: {
		...
		increaseTime(type, i) {
			if (
				type === 'start' &&
				this.requests[i].startTime !== this.oldTimes.endTime &&
				new Date(this.requests[i].startTime) < new Date(this.requests[i].endTime)
			) {
				...
				type === 'end' &&
				this.requests[i].endTime !== this.oldTimes.endTime &&
				new Date(this.requests[i].endTime) >= new Date(this.requests[i].startTime)
			) {
		...
		decreaseTime(type, i) {
			if (
				type === 'start' &&
				this.requests[i].startTime !== this.oldTimes.startTime &&
				new Date(this.requests[i].startTime) <= new Date(this.requests[i].endTime)
			) {
				let d = new Date(this.requests[i].startTime);
				d.setUTCMinutes(d.getUTCMinutes() - 15);
				...
				type === 'end' &&
				this.requests[i].endTime !== this.oldTimes.startTime &&
				new Date(this.requests[i].endTime) > new Date(this.requests[i].startTime)
			) {
				let d = new Date(this.requests[i].endTime);
				d.setUTCMinutes(d.getUTCMinutes() - 15);
